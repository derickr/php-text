...
#define PHP_ICU_TEXT_CACHE_NOVALUE -1
#define PHP_ICU_TEXT_CACHE_UNSET(t) ((t)->size_cache = PHP_ICU_TEXT_CACHE_NOVALUE)

struct _php_icu_text {
	UChar        *tval;
	int32_t       tlen; /* Without trailing \0 */
	int32_t       bufs; /* Buffer size of UChars in *val */
	int32_t       size_cache; /* Cache of number of graphemes in string */
	uint32_t      rc;
};
...
#define PHP_ICU_TEXT_ALLOC(t,l) { (t)->tval = ecalloc(sizeof(UChar), (l)+1); (t)->bufs = (l); PHP_ICU_TEXT_CACHE_UNSET(t); }
...
#define PHP_ICU_TEXT_INIT(t) { t = ecalloc(1, sizeof(struct _php_icu_text)); (t)->rc = 1; PHP_ICU_TEXT_CACHE_UNSET(t); }
